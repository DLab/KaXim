...
Signature::Site::Site(const string& nme,small_id _id) :
		name(nme),id(_id),acceptBinding(false){}
Signature::Site::~Site(){}

void Signature::Site::enableBinding() {
	acceptBinding = true;
}

small_id Signature::Site::getId() const {
	return id;
}
...
///EmptySite
Signature::EmptySite::EmptySite(const string& nme,small_id id) : Site(nme,id){}
Signature::EmptySite::~EmptySite(){
	if(!acceptBinding)
		ADD_WARN_NOLOC("The site "+name+" was never used.");
}
...
char Signature::EmptySite::getType() const {
	return BIND;
}
...
///LabelSite
Signature::LabelSite::LabelSite(const string& nme,small_id id) : Site(nme,id),labels(){ }
...
char Signature::LabelSite::getType() const {
	return acceptBinding ? LABEL | BIND : LABEL;
}


///RangeSite
template <typename T>
Signature::RangeSite<T>::RangeSite(const string &nme,small_id id) : Site(nme,id){};
...
template <typename T>
char Signature::RangeSite<T>::getType() const {
	return acceptBinding ? VALUE | BIND :  VALUE;
}

template <>
bool Signature::RangeSite<FL_TYPE>::isPossibleValue(const state::SomeValue &val) const {
	auto f_val = val.valueAs<FL_TYPE>();
	return f_val >= min && f_val <= max;
}
template <>
